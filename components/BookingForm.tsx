...
export default function BookingForm() {
  function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    const subject = `CAOS — Contratación — ${artist}`;
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-10">
      <fieldset className="grid sm:grid-cols-2 gap-x-8 gap-y-8">
        ...
        <div><label className={labelClass} htmlFor="email">Correo electrónico</label><input className={fieldClass} id="email" name="Correo electrónico" type="email" required /></div>
      ...
      <fieldset className="grid sm:grid-cols-2 gap-x-8 gap-y-8">
        ...
        <div><label className={labelClass} htmlFor="eventType">Tipo de evento</label><input className={fieldClass} id="eventType" name="Tipo de evento" type="text" placeholder="Sala, festival, privado, marca..." /></div>
