...
const links = [["/artists","ARTISTAS"],["/records","RECORDS"],["/about","NOSOTROS"],["/contact","CONTACTO"]];
export default function Navbar(){
 ...
 return <header className="site-header"><Link href="/" className="brand-link" aria-label="CAOS Records — inicio"><img src="/logo-white.png" width="76" height="76" alt="CAOS Records"/></Link><nav className="desktop-nav" aria-label="Navegación principal">{links.map(([href,label])=><Link href={href} key={href} aria-current={path===href?'page':undefined}>{label}</Link>)}</nav><Link href="/booking" className="nav-booking">BOOKING <span>↗</span></Link><button ref={toggle} className="menu-toggle" aria-label={open?'Cerrar menú':'Abrir menú'} aria-expanded={open} aria-controls="mobile-nav" onClick={()=>setOpen(!open)}>{open?'✕':'☰'}</button>{open&&<nav ref={menu} className="mobile-nav" id="mobile-nav" aria-label="Navegación móvil">{[...links,["/booking","BOOKING"]].map(([href,label])=><Link onClick={()=>setOpen(false)} key={href} href={href}>{label}<span>↗</span></Link>)}</nav>}</header>;
}
